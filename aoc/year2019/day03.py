...
def is_cross(a, b):
    ...
    if va is None or vb is None:
        ...
        if va:
            if (
                min(b1[0], b2[0]) <= a1[0]
                and a1[0] <= max(b1[0], b2[0])
                and (min(a1[1], a2[1]) <= b1[1] and b1[1] <= max(a1[1], a2[1]))
            ):
                return (a1[0], b1[1])
        else:
            if (
                min(a1[0], a2[0]) <= b1[0]
                and b1[0] <= max(a1[0], a2[0])
                and (min(b1[1], b2[1]) <= a1[1] and a1[1] <= max(b1[1], b2[1]))
            ):
                return (b1[0], a1[1])
    return None
...
def part2(in_data, test=False):
    ...
    for line in data:
        ...
        line_points.append(previous_struct)
        for direction, value in line:
    ...
    for i1 in range(len(points[0]) - 1):
        for i2 in range(len(points[1]) - 1):
            ...
            b1 = points[1][i2][0]
            b2 = points[1][i2 + 1][0]
            b1_len = points[1][i2][1]
            b2_len = points[1][i2 + 1][1]
            cross = is_cross((a1, a2), (b1, b2))
            if cross is not None:
                log.debug(cross)
