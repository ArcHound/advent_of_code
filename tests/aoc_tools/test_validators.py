...
from aoc_tools import validators
...
def test_validate_day(mock_datetime):
    ...
    for case in cases:
        try:
            output = validators.validate_day(**case["input"])
...
def test_validate_day_simple():
    ...
    for case in cases:
        try:
            output = validators.validate_day_simple(**case["input"])
            assert output == case["output"], "case '{}', output: exp {}, got {}".format(
                case["label"], case["output"], output
            )
        except Exception as e:
...
def test_validate_year(mock_datetime):
    ...
    for case in cases:
        try:
            output = validators.validate_year(**case["input"])
            assert output == case["output"], "case '{}', output: exp {}, got {}".format(
                case["label"], case["output"], output
            )
        except Exception as e:
